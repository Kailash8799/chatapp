...
if($_SERVER['REQUEST_METHOD']=='POST'){
    ...
    if($result->num_rows ===0){
        ...
        $result=$link->query($sql);
        if($result){
            $mess="your Account has been created successfuly please login <a href='login.php'>here</a>";
            $check=true;

        }
    }else{
        $mess="This Username or email is already taken!!";
        $check=true;
        $color="danger";
    }
}
...
<body>
    <div class="login-form mt-5">
        <form action="signup.php" method="post">
            ...
            <?php
            if($check){
           echo ' <p class="text-'.$color.'">'.$mess.'</p>';
            }
            ?>
            <div class="form-group">
        ...
        <p class="text-center text-muted small">already have an account? <a href="login.php">Sign in here!</a></p>
    </div>
</body>
